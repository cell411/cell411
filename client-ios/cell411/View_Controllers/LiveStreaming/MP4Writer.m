...
- (void) stopWriting {
    ...
    [self.writer finishWritingWithCompletionHandler:^{
        //
        NSLog(@"Stopped writing, Saving video to photo album");
        ...
        if (paths.count) {
            NSURL *localFileUrl = paths.lastObject;
            localFileUrl = [localFileUrl URLByAppendingPathComponent:MP4Filename];
...
- (void) appendAudioSample:(CMSampleBufferRef)audioSample {
    ...
    if (self.audioWriterInput.readyForMoreMediaData && self.writer.status == AVAssetWriterStatusWriting) {
        ...
        NSLog(@"appendAudioSample - sample not appended;  readyForMoreMediaData = %d, status = %ld", self.audioWriterInput.readyForMoreMediaData, (long)self.writer.status);
    }
}
