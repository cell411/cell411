...
public abstract class ImageViewTouchBase extends AppCompatImageView implements IDisposable {
  ...
  private                boolean                  isImageScalled             = false;
  ...
  public boolean isImageScalled()
  {
    return isImageScalled;
  }

  public void setImageScalled(boolean isImageScalled)
  {
    this.isImageScalled = isImageScalled;
  }
   ...
   */
  @Override public void setImageBitmap(final Bitmap bitmap)
  {
  ...
  @Override public void setImageMatrix(Matrix matrix)
  {
    ...
    boolean needUpdate = matrix == null && !current.isIdentity() || matrix != null && !current.equals(matrix);
  ...
  @SuppressLint("Override") public float getRotation()
  {
    return 0;
  }
  ...
  @Override protected void onLayout(boolean changed, int left, int top, int right, int bottom)
  {
    if (LOG_ENABLED) {
      Log.e(LOG_TAG,
            "onLayout: " + changed + ", bitmapChanged: " + mBitmapChanged + ", scaleChanged: " + mScaleTypeChanged);
    }
    ...
    if (drawable != null) {
      if (changed || mScaleTypeChanged || mBitmapChanged) {
        ...
        if (scale > getMaxScale() || scale < getMinScale()) {
          // if current scale is outside the min/max bounds
  ...
  protected void scrollBy(float distanceX, float distanceY, final double durationMs)
  {
    ...
    BaseApp.smHandler.post(new Runnable() {
      double old_x = 0;
      double old_y = 0;
  ...
  protected void zoomTo(float scale, float centerX, float centerY, final float durationMs)
  {
    if (scale > getMaxScale()) {
      scale = getMaxScale();
      Log.i("Max Scale", String.valueOf(scale));
    }
    final long startTime = System.currentTimeMillis();
  ...
  public enum DisplayType {
    ...
    NONE,
    /**
     * Image will always be presented using this view's bounds
     */
    FIT_TO_SCREEN,
    /**
