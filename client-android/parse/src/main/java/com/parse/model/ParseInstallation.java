...
import com.parse.controller.ParseCurrentInstallationController;
...
public class ParseInstallation extends ParseObject {
  ...
  private static final List<String> READ_ONLY_FIELDS = Collections.unmodifiableList(
      Arrays.asList(KEY_DEVICE_TYPE, KEY_INSTALLATION_ID, KEY_DEVICE_TOKEN, KEY_PUSH_TYPE, KEY_TIME_ZONE, KEY_LOCALE,
                    KEY_APP_VERSION, KEY_APP_NAME, KEY_PARSE_VERSION, KEY_APP_IDENTIFIER, KEY_OBJECT_ID));
  ...
  @Override
    public Task<Void> handleFetchResultAsync(final ParseObject.State newState) {
    return super.handleFetchResultAsync(newState).onSuccessTask(
        task -> getCurrentInstallationController().setAsync(ParseInstallation.this));
  }
  @Override
  ...
  private void updateVersionInfo() {
    synchronized (mutex) {
      try {
        ...
        @SuppressWarnings("deprecation") String appVersion = String.valueOf(pkgInfo.versionCode);
