...
import com.parse.controller.ParseDefaultACLController;
...
public class ParseACL implements Parcelable {
  ...
  /* package */
  public ParseACL(Parcel source, ParseParcelDecoder decoder) {
   ...
   *                                 setDefaultACL
   *                                 has been called. This value will be copied and used as a template for the
   *                                 creation of
   *                                 new ACLs, so changes to the instance after {@code setDefaultACL(ParseACL,
   *                                 boolean)}
   ...
   *                                 provided ACL will be used without modification. If acl is {@code null}, this
   *                                 value is
   *                                 ignored.
   */
    ...
    for (String key : ParseJSONUtils.keys(object)) {
      if (key.equals(UNRESOLVED_USER_JSON_KEY)) {
        ...
        try {
          ...
          throw new RuntimeException("could not decodeObject ACL: " + e.getMessage());
        }
      }
    }
    return acl;
  ...
  private boolean isUnresolvedUser(ParseUser other) {
    ...
    if (other == null || unresolvedUser == null)
      return false;
    return other == unresolvedUser ||
        (other.getObjectId() == null && other.getOrCreateLocalId().equals(unresolvedUser.getOrCreateLocalId()));
  }
