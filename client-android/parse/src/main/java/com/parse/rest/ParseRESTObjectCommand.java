...
public class ParseRESTObjectCommand extends ParseRESTCommand {
  ...
  )
  {
    ...
    return new ParseRESTObjectCommand(httpPath, ParseHttpMethod.GET, null, sessionToken);
  }
  ...
  )
  {
    ...
    return new ParseRESTObjectCommand(httpPath, ParseHttpMethod.POST, changes, sessionToken);
  }
  ...
  {
    String httpPath = String.format("classes/%s/%s", Uri.encode(className), Uri.encode(objectId));
    return new ParseRESTObjectCommand(httpPath, ParseHttpMethod.PUT, changes, sessionToken);
  }
  ...
  )
  {
    ...
    return new ParseRESTObjectCommand(httpPath, ParseHttpMethod.DELETE, null, sessionToken);
  }
}
