...
import com.parse.utils.Numbers;
...
class OfflineQueryLogic {
  ...
  private static Object getValue(Object container, String key, int depth) throws ParseException {
    if (key.contains(".")) {
      ...
      if (!(value == null || value == JSONObject.NULL || value instanceof Map || value instanceof JSONObject)) {
        ...
        throw new ParseException(ParseException.INVALID_QUERY, String.format("Key %s is invalid.", key));
      }
  ...
  private static KeyConstraints cleanRegexStartsWith(KeyConstraints regex) {
    ...
    String literalizedString = ((String) regex.get("$regex")).replaceAll("([^\\\\])(\\\\E)", "$1")
                                                             .replaceAll("([^\\\\])(\\\\Q)", "$1")
                                                             .replaceAll("^\\\\E", "").replaceAll("^\\\\Q", "")
                                                             .replaceAll("([^'])'", "$1''")
                                                             .replaceAll("^'([^'])", "''$1");
  ...
  private static boolean matchesRegexConstraint(Object constraint, Object value, String options) throws ParseException {
    ...
    if (!options.matches("^[imxs]*$")) {
      throw new ParseException(ParseException.INVALID_QUERY, String.format("Invalid regex options: %s", options));
    }
  ...
  private static boolean matchesWithinConstraint(Object constraint, Object value) throws ParseException {
    ...
    @SuppressWarnings("unchecked") HashMap<String, ArrayList<ParseGeoPoint>> constraintMap
    ...
    return (target.getLatitude() >= southwest.getLatitude() && target.getLatitude() <= northeast.getLatitude() &&
        target.getLongitude() >= southwest.getLongitude() && target.getLongitude() <= northeast.getLongitude());
  }
  ...
  private static boolean matchesGeoIntersectsConstraint(Object constraint, Object value) {
    ...
    @SuppressWarnings("unchecked") HashMap<String, ParseGeoPoint> constraintMap
  ...
  private static boolean matchesGeoWithinConstraint(Object constraint, Object value) {
    ...
    @SuppressWarnings("unchecked") HashMap<String, List<ParseGeoPoint>> constraintMap
    ...
    switch (operator) {
      ...
      default:
        throw new UnsupportedOperationException(
            String.format("The offline store does not yet support the %s operator.", operator));
    }
  }
  ...
  private static Task<Void> fetchIncludeAsync(
      final OfflineStore store, final Object container, final String path, final ParseSQLiteDatabase db
  )
  {
    ...
    if (path == null) {
      if (JSONObject.NULL.equals(container)) {
        ...
        return Task.forError(
            new ParseException(ParseException.INVALID_NESTED_KEY, "include is invalid for non-ParseObjects"));
      }
    }
  ...
  static <T extends ParseObject> Task<Void> fetchIncludesAsync(
      final OfflineStore store, final T object, ParseQuery.State<T> state, final ParseSQLiteDatabase db
  )
  {
  ...
  )
  {
    ...
    @SuppressWarnings("unchecked") ParseQuery.State<T> query = ((ParseQuery.State.Builder<T>) constraint).build();
  ...
  )
  {
    ...
    // TODO(grantland): Convert builder to state t6941155
    @SuppressWarnings("unchecked") ParseQuery.State<T> query = ((ParseQuery.State.Builder<T>) constraintMap.get(
        "query")).build();
  ...
  private <T extends ParseObject> ConstraintMatcher<T> createMatcher(
      ParseUser user, final String operator, final Object constraint, final String key,
      ...
      default:
        ...
        return new ConstraintMatcher<T>(user) {
          ...
          public Task<Boolean> matchesAsync(T object, ParseSQLiteDatabase db) {
            try {
              Object value = getValue(object, key);
              return Task.forResult(matchesStatelessConstraint(operator, constraint, value, allKeyConstraints));
            } catch (ParseException e) {
  ...
  )
  {
    ...
    for (final String key : queryConstraints.keySet()) {
      ...
      if (key.equals("$or")) {
         ...
         */
        @SuppressWarnings("unchecked") ConstraintMatcher<T> matcher = createOrMatcher(user,
                                                                                      (ArrayList<QueryConstraints>) queryConstraintValue);
        matchers.add(matcher);
