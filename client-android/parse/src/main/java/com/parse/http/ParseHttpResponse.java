...
/**
 * The http response we receive from com.parse server. Instances of this class are not immutable. The
...
public final class ParseHttpResponse {
  ...
  public static ParseHttpResponse getResponse(Response okHttpResponse) {
    return getBuilder(okHttpResponse).build();
  }

  public static Builder getBuilder(Response okHttpResponse)  {
    // Status code
    int statusCode = okHttpResponse.code();

    // Content
    InputStream content = Objects.requireNonNull(okHttpResponse.body()).byteStream();

    // Total size
    int totalSize = (int) Objects.requireNonNull(okHttpResponse.body()).contentLength();
    ...
    return new Builder().setStatusCode(statusCode)
                                          .setContent(content)
