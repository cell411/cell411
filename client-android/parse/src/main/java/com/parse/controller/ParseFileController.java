...
public class ParseFileController {
  ...
  public Task<ParseFile.State> saveAsync(
      final ParseFile.State state, final byte[] data, String sessionToken, ProgressCallback uploadProgressCallback,
  ...
  public Task<ParseFile.State> saveAsync(
      final ParseFile.State state, final File file, String sessionToken, ProgressCallback uploadProgressCallback,
      Task<Void> cancellationToken
  )
  {
    if (state.url() != null) { // !isDirty
  ...
  public Task<File> fetchAsync(
      final ParseFile.State state, @SuppressWarnings("UnusedParameters") String sessionToken,
  ...
  )
  {
    ...
    return Task.call(new Callable<Boolean>() {
      ...
      public Task<File> then(Task<Boolean> task) {
        ...
        // network
        final ParseFileRequest request = new ParseFileRequest(ParseHttpMethod.GET, state.url(), tempFile);

        // We do not need to delete the temp file since we always try to overwrite it
        return request.executeAsync(fileClient(), null, downloadProgressCallback, cancellationToken)
