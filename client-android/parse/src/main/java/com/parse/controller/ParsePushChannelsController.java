...
public class ParsePushChannelsController {
  ...
  public Task<Void> subscribeInBackground(final String channel) {
    ...
    return getCurrentInstallationController().getAsync()
                                             .onSuccessTask(new Continuation<ParseInstallation, Task<Void>>() {
                                               @Override
                                               public Task<Void> then(Task<ParseInstallation> task) {
                                                 ParseInstallation installation = task.getResult();
                                                 List<String> channels = installation.getList(
                                             ...
                                             .onSuccessTask(new Continuation<ParseInstallation, Task<Void>>() {
                                               ...
                                               public Task<Void> then(Task<ParseInstallation> task) {
                                                 ...
                                                 if (channels != null && channels.contains(channel)) {
                                                   installation.removeAll(ParseInstallation.KEY_CHANNELS,
