...
public class NetworkUserController implements ParseUserController {
  ...
  )
  {
    ...
    ParseRESTCommand command = ParseRESTUserCommand.signUpUserCommand(objectJSON, sessionToken, revocableSession);
    ...
    return command.executeAsync(client).onSuccess(new Continuation<JSONObject, ParseUser.State>() {
      ...
      public ParseUser.State then(Task<JSONObject> task) {
        ...
        return coder.decode(new ParseUser.State.Builder(), result, ParseDecoder.get()).isComplete(false).isNew(true)
                    .build();
      }
    });
  }
  ...
  {
    ParseRESTCommand command = ParseRESTUserCommand.logInUserCommand(username, password, revocableSession);
    return command.executeAsync(client).onSuccess(new Continuation<JSONObject, ParseUser.State>() {
      @Override
      public ParseUser.State then(Task<JSONObject> task) {
        JSONObject result = task.getResult();

        return coder.decode(new ParseUser.State.Builder(), result, ParseDecoder.get()).isComplete(true).build();
      }
    });
  }
  ...
  {
    JSONObject objectJSON = coder.encode(state, operations, PointerEncoder.get());
    final ParseRESTUserCommand command = ParseRESTUserCommand.serviceLogInUserCommand(objectJSON, state.sessionToken(),
                                                                                      revocableSession);
    ...
    return command.executeAsync(client).onSuccess(new Continuation<JSONObject, ParseUser.State>() {
      ...
      public ParseUser.State then(Task<JSONObject> task) {
        ...
        return coder.decode(new ParseUser.State.Builder(), result, ParseDecoder.get()).isComplete(isComplete)
                    .isNew(isNew).build();
      }
    });
  }
  ...
  )
  {
    ...
    return command.executeAsync(client).onSuccess(new Continuation<JSONObject, ParseUser.State>() {
      ...
      public ParseUser.State then(Task<JSONObject> task) {
        ...
        return coder.decode(new ParseUser.State.Builder(), result, ParseDecoder.get()).isComplete(true)
                    .isNew(command.getStatusCode() == STATUS_CODE_CREATED).putAuthData(authType, authData).build();
      }
    });
  }
