...
public class AudioEncoder extends Thread {
  ...
  public void run() {
    ...
    for (; ; ) {
      ...
      if (outputBufferId >= 0) {
        // mVideoOutputBuffers[outputBufferId] is ready to be processed or
        // rendered.
        ...
        //there is a bug in audio encoder- it starts to give negative values
        // after integer max size is exceeded
        ...
        long presentationTimeInMillis =
          (roundOffset + getUnsignedInt(info.presentationTimeUs)) / 1000;  //convert it to milliseconds
        //first it should be divided to 1000 and assign value to a long
        //then cast it to int -
        // Otherwise after about 35 minutes(exceeds integer max size)
        // presentationTime will be negative
        //in this assignment int presentationTime = (int)info
        // .presentationTimeUs/1000;  //convert it to milliseconds
