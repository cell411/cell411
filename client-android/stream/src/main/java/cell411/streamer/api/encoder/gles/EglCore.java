 ...
 * <p>
 * The EGLContext must only be attached to one thread at a time.  This class
 * is not thread-safe.
 */
public final class EglCore {
  /**
   * Constructor flag: surface must be recordable.  This discourages EGL
   * from using a
   * pixel format that cannot be converted efficiently to something usable
   * by the video
   * encoder.
   */
   ...
   * <p>
   *
   * @param sharedContext The activity to share, or null if sharing is not
   *                      desired.
  ...
  public EglCore(EGLContext sharedContext, int flags) {
    ...
    XLog.d(TAG, "EGLContext created, client version " + values[0]);
  }
  ...
  public static void logCurrent(String msg) {
    ...
    XLog.i(TAG, "Current EGL (" + msg + "): display=" + display + ", activity=" + context + ", surface=" + surface);
  }
  ...
  private EGLConfig getConfig(int flags, int version) {
    ...
    int[] attribList = {EGL14.EGL_RED_SIZE, 8, EGL14.EGL_GREEN_SIZE, 8, EGL14.EGL_BLUE_SIZE, 8, EGL14.EGL_ALPHA_SIZE, 8,
      ...
      EGL14.EGL_RENDERABLE_TYPE, renderableType, EGL14.EGL_NONE, 0,      // placeholder for
      // recordable [@-3]
      EGL14.EGL_NONE};
    ...
    if (!EGL14.eglChooseConfig(mEGLDisplay, attribList, 0, configs, 0, configs.length, numConfigs, 0)) {
      XLog.w(TAG, "unable to find RGB8888 / " + version + " EGLConfig");
      return null;
    }
    return configs[0];
  }
  ...
  @Override protected void finalize() throws Throwable {
    try {
      if (mEGLDisplay != EGL14.EGL_NO_DISPLAY) {
        ...
        // the EGL state, so if a surface or activity is still current on
        // another
        // thread we can't fully release it here.  Exceptions thrown from here
        // are quietly discarded.  Complain in the log file.
        XLog.w(TAG, "WARNING: EglCore was not explicitly released -- state may be " + "leaked");
        release();
      }
    } finally {
      super.finalize();
  ...
  /**
   * Makes our EGL activity current, using the supplied surface for both
   * "draw" and "read".
   */
  public void makeCurrent(EGLSurface eglSurface) {
    if (mEGLDisplay == EGL14.EGL_NO_DISPLAY) {
      // called makeCurrent() before create?
      XLog.d(TAG, "NOTE: makeCurrent w/o display");
    }
  ...
  /**
   * Makes our EGL activity current, using the supplied "draw" and "read"
   * surfaces.
   */
  ...
  /**
   * Returns the GLES version this activity is configured for (currently 2 or
   * 3).
   */
