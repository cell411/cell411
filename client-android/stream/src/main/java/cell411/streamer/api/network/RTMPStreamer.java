package cell411.streamer.api.network;
...
public class RTMPStreamer extends Handler implements IMediaMuxer {
  ...
  private final        Object           frameSynchronized = new Object();
  private final        ArrayList<Frame> audioFrameList    = new ArrayList<>();
  private final        ArrayList<Frame> videoFrameList    = new ArrayList<>();
  ...
  public boolean open(@NonNull String url) {
    ...
    if (result > 0) {
      ...
      throw new RuntimeException("Failed to open rtmp connection: "+result);
    }
    return isConnected;
  }

  public void close() {
    XLog.i(TAG, "close rtmp connection");
    isConnected = false;
    rtmpMuxer.close();
  }
   ...
   * connection can be closed by server. So we make packet ordering below
   * according packet's timestamp
   *
   * @param msg
   */
    ...
    switch (msg.what) {
      case SEND_AUDIO: {
        ...
        if ((msg.arg2 >= mLastReceivedAudioFrameTimeStamp) && (msg.arg1 > 0)) {
          ...
          // otherwise discard the packet. If we don't discard it, rtmp
          // connection totally drops
          ...
          XLog.w(TAG, "discarding audio packet because time stamp is older than last " +
                      "packet or data length equal to zero");
        }
        sendFrames();
      }
      break;
      case SEND_VIDEO: {
        ...
        if ((msg.arg2 >= mLastReceivedVideoFrameTimeStamp) && (msg.arg1 > 0)) {
          ...
          XLog.w(TAG, "discarding video packet because time stamp is older  than last " +
            "packet or data lenth equal to zero");
        }
        sendFrames();
      }
      break;
  ...
  private void sendFrames() {
    ...
    // we do not know the audio or video frames timestamp in advance and they
    // are not
    // deterministic. So we send video frames with the timestamp is less than
      ...
      if (audioFrame.timestamp <= timestamp) {
        ...
        if (audioFrame.timestamp >= lastSentFrameTimeStamp) {
          ...
          if (isConnected) {
            ...
            if (DEBUG) {
              XLog.d(TAG,
                    "send audio result: " + result + " time:" + audioFrame.timestamp + " length:" + audioFrame.length);
            }
            if (result < 0) {
              close();
            }
      ...
      if ((frame.timestamp <= timestamp)) {
        ...
        if (frame.timestamp >= lastSentFrameTimeStamp) {
          ...
          if (isConnected) {
            ...
            if (DEBUG) {
              XLog.d(TAG, "send video result: " + result + " time:" + frame.timestamp + " length:" + frame.length);
            }
            if (result < 0) {
              close();
            }
  ...
  @Override public void setAudioEnable(boolean enable) {
    isAudioEnabled = enable;
  }

  @Override public void writeAudio(byte[] data, int size, int presentationTime) {
    ...
    if (DEBUG) {
      XLog.d(TAG, "writeAudio size: " + size + " time:" + presentationTime);
    }
  }

  @Override public void writeVideo(byte[] data, int length, int presentationTime) {
    ...
    if (DEBUG) {
      XLog.d(TAG, "writeVideo size: " + length + " time:" + presentationTime);
    }
  }

  @Override public void stopMuxer() {
    sendEmptyMessage(RTMPStreamer.STOP_STREAMING);
  }

  @Override public int getFrameCountInQueue() {
    synchronized (frameSynchronized) {
      return frameCount;
    }
  }
