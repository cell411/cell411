...
public interface ILiveVideoBroadcaster {
   ...
   *
   * @param client   the requester, who provides the Activity,
   *                 and the GLSurfaceView
   */
  void init(ILiveVideoClient client);
   ...
   *                 can be
   *                 Camera.CameraInfo.CAMERA_FACING_BACK, Camera.CameraInfo
   *                 .CAMERA_FACING_FRONT;
   */
  ...
  /**
   * Sets the display orientation of the camera for portrait or landscape
   * orientation
   */
  void setDisplayOrientation();

  /**
   * Pauses and releases the camera, it is safe to call this function in
   ...
   *
   * @param url the rtmp url which should be in form
   *            rtmp://SERVER_ADDRESS/APP_NAME/STREAM_NAME
  ...
  /**
   * Stops broadcasting to the server
   */
