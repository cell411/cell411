 ...
 */
package cell411.streamer.api.encoder.gles;
...
public class Texture2dProgram {
  ...
  // Fragment shader with a convolution filter.  The upper-left half will be
  // drawn normally,
  // the lower-swipe_right half will have the filter applied, and a thin red
  // line will be drawn
  // at the border.
  //
  // This is not optimized for performance.  Some things that might make this
  // faster:
  // - Remove the conditionals.  They're used to present a half & half view
  // with a red
  //   stripe across the middle, but that's only useful for a demo.
  // - Unroll the loop.  Ideally the compiler does this for you when it's
  // beneficial.
  // - Bake the filter kernel into the shader, instead of passing it through
  // a uniform
  //   array.  That, combined with loop unrolling, should reduce memory
  //   accesses.
  ...
  // Simple fragment shader for use with external 2D textures (e.g. what we
  // get from
  // SurfaceTexture).
  private static final String            FRAGMENT_SHADER_EXT              =
    "#extension GL_OES_EGL_image_external : require\n" + "precision mediump float;\n" +
      ...
      "    gl_FragColor = texture2D(sTexture, vTextureCoord);\n" + "}\n";
  // Fragment shader that converts color to black & white with a simple
  // transformation.
  ...
  public Texture2dProgram(ProgramType programType) {
    ...
    XLog.d(TAG, "Created program " + mProgramHandle + " (" + programType + ")");
   ...
   * <p>
   * The appropriate EGL activity must be current (i.e. the one that was
   * used to create
   * the program).
   */
  public void release() {
    XLog.d(TAG, "deleting program " + mProgramHandle);
  ...
  public void setKernel(float[] values, float colorAdj) {
    ...
    //XLog.d(TAG, "filt kernel: " + Arrays.toString(mKernel) + ", adj=" +
    // colorAdj);
  }

  /**
   * Sets the size of the texture.  This is used to find adjacent texels when
   * filtering.
   */
  public void setTexSize(int width, int height) {
    ...
    //XLog.d(TAG, "filt size: " + width + "x" + height + ": " + Arrays
    // .toString(mTexOffset));
  }
   ...
   * @param coordsPerVertex The number of coordinates per vertex (e.g. x,y
   *                        is 2).
   * @param vertexStride    Width, in bytes, of the position data for each
   *                        vertex (often
   ...
   * @param texStride       Width, in bytes, of the texture data for each
   *                        vertex.
   */
