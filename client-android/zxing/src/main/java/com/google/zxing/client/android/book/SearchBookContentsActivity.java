...
import com.google.zxing.base.BaseActivity;
...
public final class SearchBookContentsActivity extends BaseActivity {
  ...
  private final View.OnClickListener buttonListener = this::onButtonClick;
  public void onButtonClick(View view) {
    launchSearch();
  }
  private final View.OnKeyListener keyListener = this::onKeyPressed;
  private boolean onKeyPressed(final View view, final int keyCode, final KeyEvent event) {
  ...
  private static class NetworkTask extends AsyncTask<String,Object,JSONObject> {

    private final SearchBookContentsActivity mSearchBookContentsActivity;

    public NetworkTask(
      final SearchBookContentsActivity searchBookContentsActivity) {
      this.mSearchBookContentsActivity = searchBookContentsActivity;
    }
    ...
    protected void onPostExecute(JSONObject result) {
      if (result == null) {
        mSearchBookContentsActivity.headerView.setText(R.string.msg_sbc_failed);
      } else {
        handleSearchResults(result);
      }
      mSearchBookContentsActivity.queryTextView.setEnabled(true);
      mSearchBookContentsActivity.queryTextView.selectAll();
      mSearchBookContentsActivity.queryButton.setEnabled(true);
    }
    ...
    private void handleSearchResults(JSONObject json) {
      try {
        ...
        String text =
          mSearchBookContentsActivity.getString(R.string.msg_sbc_results) + " : " + count;
        mSearchBookContentsActivity.headerView.setText(text);
        if (count > 0) {
          JSONArray results = json.getJSONArray("search_results");
          SearchBookContentsResult.setQuery(mSearchBookContentsActivity.queryTextView.getText().toString());
          List<SearchBookContentsResult> items = new ArrayList<>(count);
          for (int x = 0; x < count; x++) {
            items.add(parseResult(results.getJSONObject(x)));
          }
          mSearchBookContentsActivity.resultListView.setOnItemClickListener(new BrowseBookListener(
            mSearchBookContentsActivity, items));
          mSearchBookContentsActivity.resultListView.setAdapter(new SearchBookContentsAdapter(
            mSearchBookContentsActivity, items));
        } else {
          String searchable = json.optString("searchable");
          if ("false".equals(searchable)) {
            mSearchBookContentsActivity.headerView.setText(R.string.msg_sbc_book_not_searchable);
          }
          mSearchBookContentsActivity.resultListView.setAdapter(null);
        }
    ...
    private SearchBookContentsResult parseResult(JSONObject json) {
      ...
      try {
        ...
        return new SearchBookContentsResult(mSearchBookContentsActivity.getString(R.string.msg_sbc_no_page_returned), "", "", false);
      }
      ...
      if (pageNumber == null || pageNumber.isEmpty()) {
        ...
        pageNumber = mSearchBookContentsActivity.getString(R.string.msg_sbc_page) + ' ' + pageNumber;
      }
      ...
      if (valid) {
        ...
        snippet = '(' + mSearchBookContentsActivity.getString(R.string.msg_sbc_snippet_unavailable) + ')';
      }
