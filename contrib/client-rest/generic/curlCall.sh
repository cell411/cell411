#!/bin/bash
if test -z "$PARSE_CONFIG"; then
  PARSE_CONFIG="$HOME/.parse/config.sh";
fi
set -xv
PARSE_CONFIG=${PARSE_CONFIG%.sh}
...
curlCall() {
  ...
  set -- "$@" -H "X-Parse-REST-API-Key: $PARSE_RESTAPI_KEY"
