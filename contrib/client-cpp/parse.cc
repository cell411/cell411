...
httplib::SSLClient cli("dev.copblock.app", 443);
...
int login(void) {
  ...
  params.insert(std::make_pair("username","dev1@copblock.app"));
  params.insert(std::make_pair("password","aa"));

  cli.set_ca_cert_path(CA_CERT_FILE);
  cli.enable_server_certificate_verification(true);


  if (auto res = cli.Get("/parse/login",params,defaultHeaders)) {
